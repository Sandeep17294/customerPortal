...
public class ApplicationMailer implements Serializable {
	...
	public void sendMail(String to, String subject, String body) {
		try {
			...
			PasswordRulesDTO rulesDTO = loginDetails.fetchCpSettings();
			  ...
			  logger.info("port =" +this.SMTPPort);
			  logger.info("user =" +this.SMTPUser);
			  logger.info("password =" +this.userPassword);
			...
			props.put("https.protocols", "TLSv1.1");
			...
			Session session = Session.getInstance(props, new javax.mail.Authenticator() {
				protected PasswordAuthentication getPasswordAuthentication() {
					if (getSMTPUser() == null)
						...
						return new PasswordAuthentication(getSMTPUser(), userPassword);
				}
			});
	...
	public void sendMail(String to, List cc, String subject, String body) {
		try {
			String[] tmpTo = { to };
	...
	/*
	 * public boolean sendEmail(EmailMessage message){ try{
	 * logger.info("Enter = sendEmail"); Properties props = new Properties();
	 * props.put("mail.smtp.host",this.getSMTPHost()); props.put("mail.smtp.auth",
	 * "true"); props.put("mail.smtp.port", SMTPPort);
	 * props.put("mail.smtp.ssl.enable", "true");
	 * props.put("mail.smtp.socketFactory.port", SMTPPort);
	 * props.put("mail.smtp.socketFactory.class", "javax.net.ssl.SSLSocketFactory");
	 * props.put("mail.smtp.starttls.enable", "true"); logger.info("SMTP Host = " +
	 * this.getSMTPHost()); Session session = Session.getInstance(props, new
	 * javax.mail.Authenticator() { protected PasswordAuthentication
	 * getPasswordAuthentication() { if(getSMTPUser()==null) return new
	 * PasswordAuthentication("",""); else return new
	 * PasswordAuthentication(getSMTPUser(),userPassword); } });
	 * 
	 * session.setDebug(true); Message msg = new MimeMessage(session);
	 * InternetAddress addressFrom = new InternetAddress(getFrom());
	 * msg.setFrom(addressFrom); InternetAddress[] addressTo = new
	 * InternetAddress[message.getTo().length]; logger.info("To addres List");
	 * for(int index=0;index<message.getTo().length;index++){
	 * logger.info("To address = " + addressTo[index]); addressTo[index]=new
	 * InternetAddress(message.getTo()[index]); }
	 * 
	 * msg.setRecipients(Message.RecipientType.TO, addressTo);
	 * msg.addHeader("MyHeaderName", "MIME-Version: 1.0");
	 * msg.setSubject(message.getSubject()); if(message.getAttachments().size() >
	 * 0){ BodyPart messageBodyPart = new MimeBodyPart();
	 * messageBodyPart.setText(message.getBody()); Multipart multipart = new
	 * MimeMultipart(); multipart.addBodyPart(messageBodyPart); for(String fileName
	 * : message.getAttachments()){ messageBodyPart = new MimeBodyPart(); DataSource
	 * source = new FileDataSource(fileName); messageBodyPart.setDataHandler(new
	 * DataHandler(source)); File f = new File(fileName);
	 * messageBodyPart.setFileName(f.getName());
	 * multipart.addBodyPart(messageBodyPart); } msg.setContent(multipart); }else{
	 * msg.setContent(message.getBody(), "text/plain;charset=UTF-8;format=flowed");
	 * } //msg.set
	 * 
	 * Transport.send(msg);
	 * logger.info("Email has been sent to "+addressTo.toString()); return true;
	 * }catch(AddressException adEx){ logger.info("An AddressException occurred = "
	 * + adEx.getMessage()); adEx.printStackTrace(); return false;
	 * }catch(MessagingException msgEx){
	 * logger.info("An MessagingException occurred = " + msgEx.getMessage());
	 * msgEx.printStackTrace(); return false; }catch(Exception ex){
	 * logger.info("An Exception occurred = " + ex.getMessage());
	...
	public void sendMailAttachment(String to, String subject, String body, String file, String pdfName)
		...
		try {
			 ...
			 * props.put("mail.smtp.socketFactory.port", SMTPPort);
			 * props.put("mail.smtp.socketFactory.class", "javax.net.ssl.SSLSocketFactory");
			 * props.put("mail.smtp.auth", "true"); props.put("mail.smtp.port", SMTPPort);
			 * props.put("mail.smtp.ssl.enable", "true"); Session session =
			 * Session.getInstance(props, new Authenticator() {
			 * 
			 * @Override protected PasswordAuthentication getPasswordAuthentication() {
			 * System.out.println("Authenticating"); return new
			 * PasswordAuthentication(SMTPUser, userPassword); }
			 * 
			 * });
			 */
			props.put("mail.smtp.host", this.getSMTPHost());
			...
			Session session = Session.getInstance(props, new javax.mail.Authenticator() {
				protected PasswordAuthentication getPasswordAuthentication() {
					logger.info("ENTERED INSIDE pASSWORD aUTHENTICATION");
					logger.info("UserName Inside Authenticate @@@@@@@@@@@@@@" + SMTPUser);
					logger.info("Password Inside Authenticate @@@@@@@@@@@@@@" + userPassword);
					return new PasswordAuthentication(SMTPUser, userPassword);

				}
			});

			logger.info("UserName is @@@@@@@@@@@@@@" + SMTPUser);
			logger.info("UserName is @@@@@@@@@@@@@@" + userPassword);
